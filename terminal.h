...
*
*       1.  MAIN LOOP AND INITIALIZATION:
*           - `InitTerminal()`: Sets up the default terminal state: screen buffers (arrays of `EnhancedTermChar`), cursor, modes (DECModes, ANSIModes),
*             color palettes, character sets (`CharsetState`), tab stops, keyboard (`VTKeyboard`), performance settings, and initializes the Raylib font texture.
*           - `UpdateTerminal()`: This is the main update tick. It calls `ProcessPipeline()` to handle incoming data, updates cursor and text blink timers, and flushes
*             any responses queued for the host application via the `ResponseCallback`.
*           - `DrawTerminal()`: Renders the current state of the active screen buffer to the Raylib window. It iterates through each `EnhancedTermChar` cell,
*             resolves its foreground and background colors (from `ExtendedColor`), applies attributes (bold, underline, etc.), and draws the character glyph from the
*             `font_texture`. It also handles drawing Sixel graphics and the cursor.
*
*       2.  INPUT PIPELINE AND CHARACTER PROCESSING:
*           - Data intended for the terminal (e.g., from a PTY or application simulating a host) is fed into `terminal.input_pipeline` using functions like
*             `PipelineWriteChar()`, `PipelineWriteString()`, `PipelineWriteFormat()`.
...
*           - `ProcessNormalChar()` handles printable characters by translating them through the active character set, applying current attributes, placing them on the
*             screen (considering auto-wrap and insert mode), and handling C0 control codes (like BEL, BS, LF, CR, ESC) by delegating to `ProcessControlChar()`.
*
*       3.  ESCAPE SEQUENCE PARSING:
*           - When an ESC (0x1B) is encountered, `terminal.parse_state` changes.
*           - `ProcessEscapeChar()` handles the character immediately following ESC. For example, '[' transitions to `PARSE_CSI`, ']' to `PARSE_OSC`, 'P' to `PARSE_DCS`.
*             Other characters might trigger single-character escape sequences (e.g., ESC D for IND).
*           - `ProcessCSIChar()` accumulates parameters (numbers separated by ';') into `terminal.escape_buffer` until a final command character (A-Z, a-z, @, etc.)
*             is received. `ParseCSIParams()` then converts the buffered string into numeric parameters, and `ExecuteCSICommand()` dispatches to the specific handler for
...
*               // Render the terminal
*               SituationBeginFrame();
*                   SituationCmdClearBackground(BLACK); // Or your desired background color
*                   DrawTerminal();
*               SituationEndFrame();
*           - On exit: `CleanupTerminal()` and `SituationShutdown()`.
*
*       2.  SENDING DATA TO THE TERMINAL (Simulating Host Output):
...
*
*       3.  RECEIVING RESPONSES AND KEY EVENTS FROM THE TERMINAL:
...
*
*       4.  CONFIGURING TERMINAL BEHAVIOR:
*           - VT Compliance Level:
